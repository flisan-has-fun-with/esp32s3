...
pub fn build() {
    ...
    let write_dir = format!("{manifest_path}/image/value").parse::<PathBuf>().unwrap();

    std::fs::create_dir_all(&write_dir).unwrap();

    let mut image_rs_content = format!(r"
#[allow(unused)]
...
",
    ...
    for image in glob(&format!("{manifest_path}/image/*.png")).unwrap() {
        ...
        std::fs::write(
            write_dir.join(format!("{image_name}.bin")),
            &buffer_value
        ).unwrap();

        image_rs_content.push_str(
            &format!(
